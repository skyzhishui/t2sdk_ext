...
{	
	...
	Route_Info routeInfo;//20110302 xuxp 请求里面增加路由信息d
...
public:
    ...
    */
	...
	typedef struct tagRespondData
{
	int functionID;//返回的功能号
	int returnCode;//返回码
	int errorNo;//错误号
	char* errorInfo;//错误信息
	int issueType;//MC1.0的订阅类型
	void* lpKeyInfo;//MC1.0的消息关键字段，这是一个pack的二进制格式
	int keyInfoLen;//MC1.0的关键字段长度
	Route_Info sendInfo;//20110302 xuxp 应答里面增加发送者信息
}RET_DATA, *LPRET_DATA;
...
CConnectionInterface* FUNCTION_CALL_MODE NewConnection(CConfigInterface *lpConfig);
	...
	* @return实现CConnectionInterface接口的类的指针
	*/
CConnectionInterface* FUNCTION_CALL_MODE NewConnectionEx(CConfigInterface *lpConfig, int nReserved = 0);
/**
